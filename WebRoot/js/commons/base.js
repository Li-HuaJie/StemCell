...
var sc={
	...
	checkLogin:function(data){//检查是否登录超时
		if(data.logoutFlag){
			sc.closeProgress();
			sc.alert('提示',"登录超时,点击确定重新登录.",'error',sc.toLogin);
			return false;
		}
		return true;
	},
			 	...
			 	error:function(response, textStatus, errorThrown){
			 		try{
				 		...
				 		if(!sc.checkLogin(data)){
					 		...
					 		sc.alert('提示', data.msg || "请求出现异常,请联系管理员",'error');
					 	}
			 		}catch(e){
			 			alert(e);
			 			sc.alert('提示',"请求出现异常,请联系管理员1",'error');
			 		}
			 	},
			 	complete:function(){
	...
	submitForm:function(form,callback,dataType){
			var option =
			{
			 ...
			 sc.ajaxSubmit(form,option);
	},
	saveForm:function(form,callback){
		if(form.form('validate')){
			sc.progress('Please waiting','Save ing...');
			//ajax提交form
	...
	getById:function(url,option,callback){
		sc.progress();
		sc.ajaxJson(url,option,function(data){
			sc.closeProgress();
			if(data.success){
				if(callback){
			       	callback(data);
