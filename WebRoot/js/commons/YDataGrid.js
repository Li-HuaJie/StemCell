var YDataGrid = function(config){
		...
		var Handler = {
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					sc.getById(Action.getId,data,function(result){
					...
					$.messager.confirm('Confirm','Are you sure you want to delete record?',function(r){  
					    if (r){
					    	...
					    	var  data = $("input[name='"+idKey+"']", Form.list ).fieldSerialize(); //序列化字段
					   		sc.deleteForm(Action.remove,data,function(result){
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
									callback(result);
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					sc.saveForm(Form.edit,function(data){
		...
		var initTbar = function(){
			...
			//查询页面授权的btnType
			sc.ajaxJson(_url,data,function(data){
				if(data.success){
					if(data.allType){
