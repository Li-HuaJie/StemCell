...
//	private final static String sc_LOGO_BG ="d:\\t2\\watermark_bg.png";
//	private final static String sc_LOGO_LOGO = "d:\\t2\\watermark_logo.png";
	private final static String sc_LOGO_BG = PathUtil.getRootPath()+"/static/common/images/watermark_bg.png" ; //水印背景图片 白色半透明
	private final static String sc_LOGO_LOGO = PathUtil.getRootPath()+"/static/common/images/watermark_logo.png" ;//水印图片 名称+网址
	...
	public void create(boolean isAddLogo) throws Exception{
        ...
        try{   
	        ...
	        //添加水印 start
	 		File fileBg =  new File(sc_LOGO_BG);
	 		File fileLogo =  new File(sc_LOGO_LOGO);
	 		if(fileBg.exists() && fileLogo.exists() && isAddLogo){
	 			mask = new MagickImage(new ImageInfo(sc_LOGO_BG));  
	 			image.compositeImage(CompositeOperator.AtopCompositeOp, mask,0, height-30);  
	 			mask = new MagickImage(new ImageInfo(sc_LOGO_LOGO));  
	 			image.compositeImage(CompositeOperator.AtopCompositeOp, mask, 0,  height-30);  
	 		}
	 		//添加水印 end
